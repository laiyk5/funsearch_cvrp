...
The CVRP was first introduced by \citet{dantzig1959}. Since then, a large number of heuristic algorithms have been proposed. The Nearest Neighbor heuristic iteratively selects the closest unvisited customer, which is simple but easily trapped in local optima. The Clarke-Wright Savings algorithm merges routes based on cost savings, achieving better performance in many scenarios. The Sweep algorithm clusters customers by polar angles and constructs routes sequentially.

Recently, learning-based heuristics have emerged. Many methods use reinforcement learning or graph neural networks to learn improvement policies. However, these methods require extensive training data and computational resources.

FunSearch is a sample-efficient framework that uses LLMs to directly generate executable algorithms. It has achieved impressive results in mathematical problems and combinatorial optimization. To the best of our knowledge, few studies apply Fun Search to automatically generate CVRP heuristics with practical optimization strategies for API stability and efficient search. This paper fills this gap.
